...
export function BrowserWindow({
  ...
  const sizeClasses = {
    ...
    auto: "h-auto min-h-[16rem] max-w-3xl",
  };
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex flex-1", size === "auto" ? "min-h-0" : "h-0")}>
        {/* Left Sidebar */}
        {showSidebar && sidebarPosition === "left" && (
          <div
            className={`border-foreground/5 bg-muted/20 border-r ${sidebarSizes[size === "auto" ? "md" : size]} h-full flex-shrink-0`}
          >
        ...
        {/* Main Content Area */}
        <div className={cn("relative min-w-0 flex-1", size === "auto" ? "h-auto" : "h-full")}>
          {children}
        </div>
        ...
        {showSidebar && sidebarPosition === "right" && (
          <div
            className={`border-foreground/5 bg-muted/20 border-l ${sidebarSizes[size === "auto" ? "md" : size]} h-full flex-shrink-0`}
          >
            <SidebarContent items={sidebarItems} />
          </div>
        )}
